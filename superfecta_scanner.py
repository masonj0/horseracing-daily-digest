...
import sys
import time
# Windows-only; guard import for cross-platform
try:
...
def parse_harness_races(html_content, source_url, country="AU"):
    ...
    try:
        ...
        for a in soup.find_all("a", href=True):
            href = a["href"]
            if "race-fields" in href or "entries" in href or "meeting" in href:
...
def parse_harness_meeting(html_content, meeting_url, country):
    ...
    try:
        ...
        for h in soup.find_all(["h1", "h2", "h3"]):
            ...
            if text and len(text.split()) <= 6:  # Likely a track name
                track = text
                break
...
def save_enhanced_output_files(all_races, filtered_races):
    ...
    try:
        ...
        # Auto-open in browser (skip in headless)
        try:
...
def get_url_with_interactive_timeout(timeout=2):
    """Interactive URL input with timeout (Windows optimized). Returns None on non-Windows."""
    if not HAVE_MSVCRT:
        # Non-Windows: skip interactive prompt
        return None
...
def main():
    ...
    print("\n🏁 Scan complete!")
    # Avoid blocking when run non-interactively
    if sys.stdin.isatty():
        try:
            input("\nPress Enter to exit...")
        except Exception:
